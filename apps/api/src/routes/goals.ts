...
export const goalRoutes = new Elysia({ prefix: '/goals' })
  ...
  .get('/:id/projection', async ({ params, query, set }) => {
    const { id } = params
    // TODO: Get user ID from JWT token
    const userId = '1' // Mock user ID
    ...
    try {
      ...
      const projection = {
        ...
        scenarios: [
          { monthlyAmount: goal.monthlyContribution, completionDate: '2024-06-30', probability: 85 },
          { monthlyAmount: suggestedMonthly, completionDate: '2024-06-15', probability: 95 },
          { monthlyAmount: goal.monthlyContribution + 100, completionDate: '2024-05-30', probability: 98 }
        ]
      }
