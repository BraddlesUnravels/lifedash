  ...
  .get(
    ...
    async () => {
      ...
      try {
        ...
        const currentBudget =
          mockBudgets.find((b) => b.userId === userId && b.period === currentPeriod) ||
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      try {
        ...
        const existingBudget = mockBudgets.find(
          (b) => b.userId === userId && b.period === body.period,
        );
        if (existingBudget) {
          set.status = 409;
          return {
  ...
  .put(
    ...
    async ({ params, body, set }) => {
      ...
      try {
        ...
        // Update budget
        const currentBudget = mockBudgets[budgetIndex]!;
        mockBudgets[budgetIndex] = {
          id: currentBudget.id,
          userId: currentBudget.userId,
          period: body.period || currentBudget.period,
          amountTotal:
            body.amountTotal !== undefined ? body.amountTotal : currentBudget.amountTotal,
          amountSpent: currentBudget.amountSpent,
